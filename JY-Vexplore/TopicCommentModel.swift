...
class BaseTopicCommentModel: NSObject
{
    ...
    fileprivate func getContentAttributedString(withNodes nodes: [HTMLNode]) -> NSMutableAttributedString
    {
        ...
        for node in nodes
        {
            ...
            {
                let clearedContent = content.stringbyRemoveingNewLinesAndWhitespace()
                //content.stringByRemovingNewLinesAndWhitespace()
            ...
            else if node.tag == "a", let content = node.content, let url = node["href"]
            {
                ...
                if content.isEmpty == false
                {
                    ...
                    attr.setHighlightText(withcolor: UIColor.href, url: url)
...
class TopicCommentModel: BaseTopicCommentModel
{
    ...
    func getUsersInComment() -> Set<String>
    {
        ...
        contentAttributedString.enumerateAttribute(HighlightAttributedName, in: NSMakeRange(0, contentAttributedString.length), options: []) {(attribute, range, stop) -> Void in
            if let url = attribute as? String
            {
                let result = URLAnalysisResult(url: url)
                if result.type == .memeber, let username = result.value
                {
